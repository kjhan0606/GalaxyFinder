...
```makefile
OPT = -O3 -qopenmp -DINDEX -DVarPM -DXYZDBL -DNENER=0 -DNPRE=8 -DREAD_SINK -DNCHEM=3
```

**N-body only (no baryons)**:
```makefile
OPT = -O3 -qopenmp -DINDEX -DXYZDBL -DNBODY
```

**Debug build**:
```makefile
OPT = -g -O0 -qopenmp -DINDEX -DVarPM -DXYZDBL -DDEBUG=1 -DLOG=1
```

### 6.3 Recompilation After Changes
...
After modifying only Makefile flags:
