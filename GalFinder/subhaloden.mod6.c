...
Box foffindbox(FoFTPtlStruct *, int);
#include "header.h"
...
int  MergingPeak(SimpleBasicParticleType *bp,int np,Coretype *core,int numcore, int iflag){
	...
	FoFTStruct *TREE = (FoFTStruct *)Malloc(sizeof(FoFTStruct)*MAX(10000,numcore),PPTR(TREE));
	if(iflag ==0) { 
	...
	Box box = foffindbox(ptl,numcore);
	FoF_Make_Tree(TREE,ptl,numcore,box);
...
#pragma omp parallel for private(i,j)
		...
		TREE = (TStruct *) Malloc(sizeof(TStruct)*MAX(10000,np*0.5),PPTR(TREE)); 
...
#pragma omp parallel for private(i,j) reduction(+:penergy[:nend])
	    ...
	    TREE = (TStruct *) Malloc(sizeof(TStruct)*MAX(10000,snp*0.5),PPTR(TREE)); 
...
void MemberStarFoF(SimpleBasicParticleType *bp,int np, int numcore, Coretype *core){
	...
	TREE = (FoFTStruct *)Malloc(sizeof(FoFTStruct)*MAX(10000,np*0.5),PPTR(TREE));
	for(i=0;i<numcore;i++){
		num = 0;
		for(j=0;j<np;j++){
		...
		box = foffindbox(ptl,num);
		FoF_Make_Tree(TREE,ptl,num,box);
...
#ifdef DEBUG
		printf("C%d 's # of star members changes from %d to %d\n",i,num,k);
#endif
	}
	Free(TREE);Free(linked);Free(ptl);
}
