...
int main(int argc, char *argv[]) {
	...
	*/
	...
	INT8 size = NMEG * MEGABYTE;
	if(myid ==0) size = 20000*MEGABYTE;
	if(Make_Total_Memory(size )==0){
	...
	{	
		if(myid == motherrank) {
			...
			while(fread(&haloq,sizeof(HaloQ),1,rhfp) == 1){
				...
				numhalo ++;
				if(numhalo%1000==0) 
					printf("Now passing through %d:   %d for snp= %ld\n",numhalo, snend, snp);
				/*
...
//				if(snp == 165557336)
				...
				{
					if(snp >= 100000) 
						printf("Now passing throught %d:   %d\n",numhalo, haloq.np);
					do {
